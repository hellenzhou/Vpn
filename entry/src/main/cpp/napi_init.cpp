...
#define NETMANAGER_VPN_LOGE(fmt, ...) \
    OH_LOG_Print(LOG_APP, LOG_INFO, VPN_LOG_DOMAIN, VPN_LOG_TAG, "ZHOUB vpn [%{public}s %{public}d] " fmt, MAKE_FILE_NAME, __LINE__, ##__VA_ARGS__)
...
// 🐛 修复：使用原子变量保护多线程访问的计数器
static std::atomic<int> g_packetsSent{0};  // 发送的数据包计�?static std::atomic<int> g_responsesReceived{0};  // 接收的响应计�?static std::atomic<time_t> g_lastResponseTime{0};  // 最后一次收到响应的时间
static std::atomic<int> g_ipv4Packets{0};  // IPv4数据包计�?static std::atomic<int> g_ipv6Packets{0};  // IPv6数据包计�?static std::atomic<int> g_ipv4TcpPackets{0};  // IPv4 TCP数据包计�?static std::atomic<int> g_ipv6TcpPackets{0};  // IPv6 TCP数据包计�?static std::atomic<int> g_httpPackets{0};  // HTTP数据包计�?(端口80)
static std::atomic<int> g_httpsPackets{0};  // HTTPS数据包计�?(端口443)
static std::atomic<int> g_detailedLogCount{0};  // 详细日志计数器（仅记录前20个HTTP/HTTPS连接�?static std::atomic<int> g_packetsReadFromTun{0};  // 从TUN读取的数据包总数
static std::atomic<int> g_packetsForwarded{0};  // 成功转发的数据包总数
static std::atomic<int> g_packetsDropped{0};  // 被丢弃的数据包总数（读取失败、发送失败等�?static std::atomic<int> g_packetsSendFailed{0};  // 发送失败的数据包数
static std::atomic<time_t> g_vpnStartTime{0};  // VPN启动时间
static std::atomic<int> g_trafficCheckInterval{0};  // 流量检查间隔计数器
// 获取对应字符串数�? 用于获取udp server 的IP地址
...
void HandleReadTunfd(FdInfo fdInfo)
{
    ...
    OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", "[VPN客户端] TUN读取线程已启�?tunFd=%{public}d tunnelFd=%{public}d 服务�?%{public}s:%{public}d",
                 fdInfo.tunFd, fdInfo.tunnelFd,
                 inet_ntoa(fdInfo.serverAddr.sin_addr), ntohs(fdInfo.serverAddr.sin_port));
    
    // 🔥 记录VPN启动时间，用于流量劫持检�?    g_vpnStartTime = time(nullptr);
    OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                 "🔍 [流量劫持检查] VPN启动时间: %{public}lld, 开始监控流量劫持情�?, (long long)g_vpnStartTime.load());

    uint8_t buffer[BUFFER_SIZE] = {0};
    int packetCount = 0;

    while (g_threadRunF) {
        // 检查文件描述符有效�?        if (fdInfo.tunFd < 0) {
            ...
            if (version == 4 && readResult >= 20) {  // IPv4
                ...
                if (protocol == 6) {  // TCP
                    ...
                    if (readResult >= 40) {
                        ...
                        // 🔥 ZHOUB日志：TUN设备转发到代理服务器�?                        OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                                     "[TUN→代理] 源IP:%{public}s 目的IP:%{public}s 源端�?%{public}d 目的端口:%{public}d 协议:TCP 大小:%{public}d字节",
                        ...
                        // 🔥 记录所有TCP连接（包括HTTP/HTTPS�?                        OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                                     "📊 TCP数据�?#%{public}d: %{public}s:%{public}d -> %{public}s:%{public}d%{public}s (大小=%{public}d字节)",
                        ...
                        OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                                     "[VPN客户端] 📊 数据�?#%{public}d: IPv4 TCP %{public}s:%{public}d -> %{public}s:%{public}d%{public}s",
                                     packetCount, srcIP, srcPort, dstIP, dstPort, serviceLabel);
                    }
                    ...
                    if (readResult >= 28) {
                        ...
                        // 🔥 ZHOUB日志：TUN设备转发到代理服务器�?                        OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                                     "[TUN→代理] 源IP:%{public}s 目的IP:%{public}s 源端�?%{public}d 目的端口:%{public}d 协议:UDP 大小:%{public}d字节",
                                     srcIP, dstIP, srcPort, dstPort, readResult);
                        
                        // 🔥 详细记录DNS查询（UDP端口53�?                        if (dstPort == 53) {
                            OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                                         "🔍 DNS查询请求: %{public}s:%{public}d -> %{public}s:%{public}d (UDP, 大小=%{public}d字节)",
                                         srcIP, srcPort, dstIP, dstPort, readResult);
                            OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                                         "📤 DNS查询将通过UDP隧道转发到VPN服务�?);
                        } else {
                            OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                                         "📦 UDP数据�? %{public}s:%{public}d -> %{public}s:%{public}d (大小=%{public}d字节)",
                                         srcIP, srcPort, dstIP, dstPort, readResult);
                        }
                    }
                    ...
                    // 🔥 ZHOUB日志：TUN设备转发到代理服务器�?                    OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                                 "[TUN→代理] 源IP:%{public}s 目的IP:%{public}s 源端�?0 目的端口:0 协议:ICMP 大小:%{public}d字节",
                                 srcIP, dstIP, readResult);
                    OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                                 "🏓 ICMP数据�? %{public}s -> %{public}s (大小=%{public}d字节)",
                                 srcIP, dstIP, readResult);
                    NETMANAGER_VPN_LOGI("📊 PACKET #%d: IPv4 ICMP %s -> %s", packetCount, srcIP, dstIP);
                } else {
                    // 🔥 ZHOUB日志：TUN设备转发到代理服务器�?                    OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                                 "[TUN→代理] 源IP:%{public}s 目的IP:%{public}s 源端�?0 目的端口:0 协议:%{public}d 大小:%{public}d字节",
                                 srcIP, dstIP, protocol, readResult);
                    OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                                 "📦 其他协议数据�? 协议=%{public}d %{public}s -> %{public}s (大小=%{public}d字节)",
                ...
                if (nextHeader == 6) {  // TCP
                    g_ipv6TcpPackets++;
                    OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                                 "📊 IPv6 TCP数据�?#%{public}d: %{public}s -> %{public}s (大小=%{public}d字节)",
                    ...
                    OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                                 "📊 IPv6 UDP数据�?#%{public}d: %{public}s -> %{public}s (大小=%{public}d字节)",
                                 packetCount, srcIP, dstIP, readResult);
                    NETMANAGER_VPN_LOGI("📊 PACKET #%d: IPv6 UDP %s -> %s", packetCount, srcIP, dstIP);
                } else if (nextHeader == 58) {  // ICMPv6
                    OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                                 "📊 IPv6 ICMPv6数据�?#%{public}d: %{public}s -> %{public}s (大小=%{public}d字节)",
                                 packetCount, srcIP, dstIP, readResult);
                    NETMANAGER_VPN_LOGI("📊 PACKET #%d: IPv6 ICMPv6 %s -> %s", packetCount, srcIP, dstIP);
                } else {
                    OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                                 "📊 IPv6其他协议数据�?#%{public}d: 协议=%{public}d %{public}s -> %{public}s (大小=%{public}d字节)",
                ...
                // 数据包太小或版本未知，但仍然会被转发
                OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                             "⚠️ 未知/异常数据�?#%{public}d: IP版本=%{public}d (大小=%{public}d字节) - 仍将转发",
                             packetCount, version, readResult);
                NETMANAGER_VPN_LOGI("📊 PACKET #%d: Unknown IP version %d", packetCount, version);
            }
        } else {
            // readResult < 1 的情况（理论上不会到这里，因为上面已经检查了readResult <= 0�?            OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                         "⚠️ 异常数据�?#%{public}d: 大小=%{public}d字节 (< 1字节) - 仍将转发",
                         packetCount, readResult);
        }

        // 读取到虚拟网卡的数据，通过udp隧道，发送给服务�?        if (fdInfo.tunnelFd < 0) {
            NETMANAGER_VPN_LOGE("Invalid tunnelFd: %{public}d, stopping send loop", fdInfo.tunnelFd);
            OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                         "�?隧道FD无效: tunnelFd=%{public}d, 数据�?#%{public}d 无法转发，线程退�?,
        ...
        OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                     "🔄 准备转发数据�?#%{public}d: 大小=%{public}d字节 -> VPN服务�?%{public}s:%{public}d",
                     packetCount, readResult, inet_ntoa(fdInfo.serverAddr.sin_addr), ntohs(fdInfo.serverAddr.sin_port));
        
        // 🔥 转发前检查：隧道FD和服务器地址有效�?        if (fdInfo.tunnelFd < 0) {
            OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
        ...
        // 🔥 发送前记录详细信息（IPv4和IPv6�?        // 注意：即使数据包太小或格式异常，也会被转发（因为sendto不检查内容）
        if (readResult >= 20) {
            uint8_t version = (buffer[0] >> 4) & 0x0F;
            if (version == 4) {
                uint8_t protocol = buffer[9];
                ...
                if (protocol == 17 && readResult >= 28) {  // UDP
                    ...
                    if (dstPort == 53) {
                        OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                                     "🚀 转发DNS查询: %{public}s:%{public}d -> %{public}s:%{public}d (UDP, %{public}d字节) -> VPN服务�?%{public}s:%{public}d",
                                     srcIP, srcPort, dstIP, dstPort, readResult,
                                     inet_ntoa(fdInfo.serverAddr.sin_addr), ntohs(fdInfo.serverAddr.sin_port));
                    } else {
                        OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                                     "🚀 转发UDP请求: %{public}s:%{public}d -> %{public}s:%{public}d (%{public}d字节) -> VPN服务�?%{public}s:%{public}d",
                                     srcIP, srcPort, dstIP, dstPort, readResult,
                                     inet_ntoa(fdInfo.serverAddr.sin_addr), ntohs(fdInfo.serverAddr.sin_port));
                    }
                    ...
                    OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                                 "🚀 转发TCP请求: %{public}s:%{public}d -> %{public}s:%{public}d%{public}s (%{public}d字节) -> VPN服务�?%{public}s:%{public}d",
                                 srcIP, srcPort, dstIP, dstPort, serviceType, readResult,
                                 inet_ntoa(fdInfo.serverAddr.sin_addr), ntohs(fdInfo.serverAddr.sin_port));
                } else if (protocol == 1) {  // ICMP
                    OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                                 "🚀 转发ICMP数据�? %{public}s -> %{public}s (%{public}d字节) -> VPN服务�?%{public}s:%{public}d",
                                 srcIP, dstIP, readResult,
                                 inet_ntoa(fdInfo.serverAddr.sin_addr), ntohs(fdInfo.serverAddr.sin_port));
                } else {
                    OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                                 "🚀 转发数据�? 协议=%{public}d %{public}s -> %{public}s (%{public}d字节) -> VPN服务�?%{public}s:%{public}d",
                                 protocol, srcIP, dstIP, readResult,
                                 inet_ntoa(fdInfo.serverAddr.sin_addr), ntohs(fdInfo.serverAddr.sin_port));
                }
            } else if (version == 6 && readResult >= 40) {  // IPv6
                ...
                OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                             "🚀 转发IPv6数据�? %{public}s -> %{public}s (协议=%{public}s, %{public}d字节) -> VPN服务�?%{public}s:%{public}d",
                             srcIP, dstIP, protocolName, readResult,
                             inet_ntoa(fdInfo.serverAddr.sin_addr), ntohs(fdInfo.serverAddr.sin_port));
            } else {
                // 数据包太小（< 20字节），无法解析，但仍会转发
                OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                             "🚀 转发异常数据�? 大小=%{public}d字节 (< 20字节，无法解�? -> VPN服务�?%{public}s:%{public}d",
                             readResult, inet_ntoa(fdInfo.serverAddr.sin_addr), ntohs(fdInfo.serverAddr.sin_port));
            }
        } else {
            // readResult < 20，数据包太小，但仍会转发
            OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                         "🚀 转发异常数据�? 大小=%{public}d字节 (< 20字节) -> VPN服务�?%{public}s:%{public}d",
                         readResult, inet_ntoa(fdInfo.serverAddr.sin_addr), ntohs(fdInfo.serverAddr.sin_port));
        }
        ...
        if (sendResult <= 0) {
            ...
            // 🔥 精简日志：转发失败时只打印一条错误信�?            OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
        ...
        // 🔥 精简日志：每50个数据包输出一次统计信�?        if (g_packetsSent.load() % 50 == 0) {
            g_trafficCheckInterval.fetch_add(1);
            OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB",
                         "📊 统计: 发�?%{public}d, 成功=%{public}d, 失败=%{public}d",
                         g_packetsSent.load(), g_packetsForwarded.load(), g_packetsSendFailed.load());
            
            // 🔥 流量劫持完整性检�?            time_t currentTime = time(nullptr);
            time_t vpnUptime = currentTime - g_vpnStartTime;
            OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                         "🔍 [流量劫持检查] ========== �?{public}d次检�?(VPN运行时间: %{public}lld�? ==========",
                         g_trafficCheckInterval, (long long)vpnUptime);
            ...
            if (g_ipv4TcpPackets.load() == 0 && g_ipv6TcpPackets.load() == 0) {
                ...
                OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                             "   3. 系统路由表配置错�?- 默认路由未指向vpn-tun");
                OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                             "   4. VPN连接未正确建�?- vpnConnection.create()可能失败");
                OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
            ...
            // 检�?: 是否有UDP流量（DNS等）
            int totalUdpPackets = g_ipv4Packets.load() - g_ipv4TcpPackets.load() + (g_ipv6Packets.load() - g_ipv6TcpPackets.load());
            if (totalUdpPackets == 0 && g_ipv4TcpPackets.load() == 0 && g_ipv6TcpPackets.load() == 0) {
                ...
                OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                             "�?[流量劫持检查] 检测到UDP流量：总计�?{public}d个UDP数据�?,
                             totalUdpPackets);
            }
            
            // 检�?: HTTP/HTTPS流量检�?            if (g_httpPackets == 0 && g_httpsPackets == 0 && (g_ipv4TcpPackets > 0 || g_ipv6TcpPackets > 0)) {
                OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
            ...
            // 检�?: 流量劫持完整性评�?            if (vpnUptime > 10) {  // VPN运行超过10�?                if (g_ipv4TcpPackets.load() == 0 && g_ipv6TcpPackets.load() == 0) {
                    OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                                 "�?[流量劫持检查] 结论：VPN运行%{public}lld秒，但未检测到TCP流量，流量劫持可能失�?,
                                 (long long)vpnUptime);
                    OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                                 "�?[流量劫持检查] 建议：检查VPN配置，确认trustedApplications为空，blockedApplications为空");
                } else {
                    OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                                 "�?[流量劫持检查] 结论：VPN运行%{public}lld秒，已检测到流量，流量劫持正常工�?,
                                 (long long)vpnUptime);
                }
            }
            
            // 🔥 检�?: 转发到代理服务器的完整性验�?            OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                         "🔍 [转发验证] ========== 转发到代理服务器验证 ==========");
            OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                         "🔍 [转发验证] 代理服务�? %{public}s:%{public}d",
                         inet_ntoa(fdInfo.serverAddr.sin_addr), ntohs(fdInfo.serverAddr.sin_port));
            OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                         "🔍 [转发验证] 隧道FD: %{public}d (有效=%{public}s)",
                         fdInfo.tunnelFd, fdInfo.tunnelFd >= 0 ? "�? : "�?);
            ...
            OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                         "🔍 [转发验证] 转发成功�? %.2f%%", forwardSuccessRate);
            
            // 转发完整性检�?            if (g_packetsReadFromTun.load() == 0) {
                OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                ...
                OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                             "   4. UDP socket发送失�?- 检查errno错误�?);
            } else if (g_packetsForwarded.load() == 0 && g_packetsSendFailed.load() == 0) {
                OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                             "⚠️ [转发验证] 警告：读取了%{public}d个数据包，但没有任何转发尝试",
                             g_packetsReadFromTun.load());
                OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                             "⚠️ [转发验证] 可能原因：转发逻辑未执行或提前退�?);
            } else if (forwardSuccessRate < 50.0) {
                OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                             "⚠️ [转发验证] 警告：转发成功率较低 (%.2f%%)，可能存在问�?,
                             forwardSuccessRate);
                OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                             "⚠️ [转发验证] 建议：检查网络连接和代理服务器状�?);
            } else if (forwardSuccessRate >= 99.0) {
                OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                             "�?[转发验证] 转发成功率优秀 (%.2f%%)，转发工作正�?,
                             forwardSuccessRate);
            } else {
                OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                             "�?[转发验证] 转发成功率良�?(%.2f%%)，转发基本正�?,
                             forwardSuccessRate);
            }
            
            // 检查是否有数据包遗�?            if (g_packetsReadFromTun.load() != totalProcessed) {
                int missing = g_packetsReadFromTun.load() - totalProcessed;
                OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                             "⚠️⚠️⚠️ [转发验证] 警告：有%{public}d个数据包未被处理�?,
                             missing);
                OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                             "⚠️ [转发验证] 读取=%{public}d, 已处�?%{public}d (成功=%{public}d + 失败=%{public}d)",
                             g_packetsReadFromTun.load(), totalProcessed, g_packetsForwarded.load(), g_packetsSendFailed.load());
                OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                             "⚠️ [转发验证] 这些数据包可能被遗漏，未转发到代理服务器");
            } else {
                OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                             "�?[转发验证] 完整性验证通过：所有读取的数据包都已尝试转�?);
            }
            
            // 检查响应情�?            if (g_packetsForwarded.load() > 0 && g_responsesReceived.load() == 0) {
                OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                             "⚠️ [转发验证] 警告：已转发%{public}d个数据包，但未收到任何响�?,
                             g_packetsForwarded.load());
                OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                             "⚠️ [转发验证] 可能原因：代理服务器未响应或响应处理有问�?);
            ...
            OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                         "🔍 [流量劫持检查] ==========================================");
            OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", "[VPN客户端] 🔍 转发完整性检�? 从TUN读取=%{public}d, 成功转发=%{public}d, 转发失败=%{public}d, 丢弃=%{public}d",
                         g_packetsReadFromTun.load(), g_packetsForwarded.load(), g_packetsSendFailed.load(), g_packetsDropped.load());
            
            // 🔥 关键检查：确保所有读取的数据包都被转�?            // 关系：g_packetsReadFromTun = g_packetsForwarded + g_packetsSendFailed + g_packetsDropped
            // 注意：totalProcessed已在上面定义，这里直接使�?            if (g_packetsReadFromTun.load() != totalProcessed) {
                OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                             "[VPN客户端] ⚠️⚠️⚠️ 警告：转发不完整！读�?%{public}d, 已处�?%{public}d (成功=%{public}d + 失败=%{public}d), 差异=%{public}d",
            ...
            if (g_ipv4TcpPackets.load() == 0 && g_ipv6TcpPackets.load() == 0) {
                ...
                if (g_httpPackets == 0 && g_httpsPackets == 0) {
                    ...
                    OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", "[VPN客户端] ✅✅�?已捕�?%{public}d 个HTTP/HTTPS连接�?, g_httpPackets.load() + g_httpsPackets.load());
                    OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", "[VPN客户端] ");
                    OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", "[VPN客户端] 🔥 关键测试结论�?);
                    OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", "[VPN客户端] �?如果此时浏览器无法访问网�?= VPN工作正常（流量被捕获且服务器未响应）");
                    OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", "[VPN客户端] �?如果此时浏览器仍能访问网�?= 浏览器有双路径（部分流量绕过VPN�?);
                    OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", "[VPN客户端] ");
                }
            }
        }
        ...
        if (g_packetsSent.load() > 10 && g_responsesReceived.load() == 0) {
            ...
            if (g_lastResponseTime == 0 || (now - g_lastResponseTime) > 5) {
                NETMANAGER_VPN_LOGE("⚠️ ⚠️ ⚠️ 警告：已发�?%{public}d 个数据包，但未收到任何响应！", g_packetsSent.load());
                NETMANAGER_VPN_LOGE("⚠️ 可能原因：VPN服务�?127.0.0.1:8888)未运行或未响�?);
                OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", "[VPN客户端] ⚠️ ⚠️ ⚠️ 警告：已发�?%{public}d 个数据包，但未收到任何响应！", g_packetsSent.load());
                OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", "[VPN客户端] ⚠️ 可能原因：VPN服务�?127.0.0.1:8888)未运行或未响�?);
...
void HandleTcpReceived(FdInfo fdInfo)
{
    ...
    while (g_threadRunF) {
        // 检查文件描述符有效�?        if (fdInfo.tunnelFd < 0) {
        ...
        // 🔥 详细记录接收到的响应（IPv4和IPv6�?        char srcIP[40] = {0}, dstIP[40] = {0};
        ...
        if (length >= 20) {
            ...
            if (version == 4) {
                ...
                if (protocol == 17 && length >= 28) {  // UDP
                    ...
                    if (srcPort == 53) {
                        OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                                     "📥 收到DNS响应: %{public}s:%{public}d -> %{public}s:%{public}d (UDP, %{public}d字节) <- VPN服务�?,
                                     srcIP, srcPort, dstIP, dstPort, length);
                    } else {
                        OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                                     "📥 收到UDP响应: %{public}s:%{public}d -> %{public}s:%{public}d (%{public}d字节) <- VPN服务�?,
                                     srcIP, srcPort, dstIP, dstPort, length);
                    }
                    ...
                    OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                                 "📥 收到TCP响应: %{public}s:%{public}d -> %{public}s:%{public}d%{public}s (%{public}d字节) <- VPN服务�?,
                                 srcIP, srcPort, dstIP, dstPort, serviceType, length);
                } else if (protocol == 1) {  // ICMP
                    protocolName = "ICMP";
                    OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                                 "📥 收到ICMP响应: %{public}s -> %{public}s (%{public}d字节) <- VPN服务�?,
                                 srcIP, dstIP, length);
                } else {
                    snprintf(srcIP, sizeof(srcIP), "协议%d", protocol);
                    OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                                 "📥 收到响应: 协议=%{public}d %{public}s -> %{public}s (%{public}d字节) <- VPN服务�?,
                ...
                OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                             "📥 收到IPv6响应: %{public}s -> %{public}s (协议=%{public}s, %{public}d字节) <- VPN服务�?,
        ...
        OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                     "📥 响应统计: 响应 #%{public}d 从VPN服务器收�?%{public}d 字节 (总计响应: %{public}d)",
                     responseCount, length, g_responsesReceived.load());
        ...
        OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                     "[代理→TUN] 源IP:%{public}s 目的IP:%{public}s 源端�?%{public}d 目的端口:%{public}d 协议:%{public}s 大小:%{public}d字节 数据:%{public}s",
        ...
        if (ret <= 0) {
            ...
            OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                         "�?写入TUN成功: 响应 #%{public}d 已写�?%{public}d 字节到TUN设备",
                         responseCount, ret);
        }
    }
...
{
    NETMANAGER_VPN_LOGI("========== UdpConnect() 开始执�?==========");
    
    size_t argc = 2;
    napi_value args[2] = { nullptr };
    napi_get_cb_info(env, info, &argc, args, nullptr, nullptr);
    
    NETMANAGER_VPN_LOGI("📋 参数检�? argc=%{public}zu", argc);
    if (argc < 2) {
        NETMANAGER_VPN_LOGE("�?参数不足: 需�?个参�?IP地址和端�?，实际收�?{public}zu�?, argc);
        napi_value retValue;
    ...
    NETMANAGER_VPN_LOGI("📋 解析IP地址参数: 原始字符串长�?%{public}zu", ipAddr.length());
    if (ipAddr.empty()) {
        NETMANAGER_VPN_LOGE("�?IP地址参数为空或无�?);
        napi_value retValue;
        napi_create_int32(env, -1, &retValue);
        return retValue;
    }
    ...
    NETMANAGER_VPN_LOGI("🔍 VPN服务器配�? IP=%{public}s, Port=%{public}d", ipAddr.c_str(), port);
    
    // 验证端口范围
    if (port <= 0 || port > 65535) {
        NETMANAGER_VPN_LOGE("�?端口号无�? %{public}d (有效范围: 1-65535)", port);
        napi_value retValue;
        napi_create_int32(env, -1, &retValue);
        return retValue;
    }
    ...
    if (sockFd == -1) {
        ...
        NETMANAGER_VPN_LOGE("�?错误�? errno=%{public}d", errno_save);
    ...
    if (setsockopt_result == -1) {
        ...
        NETMANAGER_VPN_LOGI("�?步骤2完成: Socket接收超时已设置为1�?);
    }
    ...
    // 转换IP地址字符串为网络字节�?    in_addr_t addr = inet_addr(ipAddr.c_str());
    if (addr == INADDR_NONE) {
    ...
    NETMANAGER_VPN_LOGI("   - IP字符�? %{public}s", ipAddr.c_str());
    NETMANAGER_VPN_LOGI("   - 网络字节序IP: %{public}s", inet_ntoa(g_fdInfo.serverAddr.sin_addr));
    NETMANAGER_VPN_LOGI("   - 端口(主机�?: %{public}d", port);
    NETMANAGER_VPN_LOGI("   - 端口(网络�?: %{public}d", ntohs(g_fdInfo.serverAddr.sin_port));

    // 步骤4: 创建返回�?    NETMANAGER_VPN_LOGI("📦 步骤4: 创建NAPI返回�?..");
    napi_value tunnelFd;
    ...
    NETMANAGER_VPN_LOGI("📝 注意: UDP是无连接协议，socket已创建但未实际连�?);
...
static napi_value StartVpn(napi_env env, napi_callback_info info)
{
    ...
    // 启动两个线程, 一个处理读取虚拟网卡的数据，另一个接收服务端的数�?    OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                 "[VPN客户端] 🚀 准备启动数据转发线程...");
    OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                 "[VPN客户端] 📋 线程参数: tunFd=%{public}d, tunnelFd=%{public}d, 服务�?%{public}s:%{public}d",
    ...
    if (g_fdInfo.tunFd < 0) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, 0x0000, "ZHOUB", 
                     "[VPN客户端] �?错误: tunFd无效 (%{public}d)，无法启动数据转发线�?, g_fdInfo.tunFd);
        NETMANAGER_VPN_LOGE("Invalid tunFd: %{public}d, cannot start VPN", g_fdInfo.tunFd);
    }
    
    if (g_fdInfo.tunnelFd < 0) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, 0x0000, "ZHOUB", 
                     "[VPN客户端] �?错误: tunnelFd无效 (%{public}d)，无法启动数据转发线�?, g_fdInfo.tunnelFd);
