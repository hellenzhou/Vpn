...
void HandleReadTunfd(FdInfo fdInfo)
{
    ...
    OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", "[VPN客户端] TUN读取线程已启动 tunFd=%{public}d tunnelFd=%{public}d 服务器=%{public}s:%{public}d",
                 fdInfo.tunFd, fdInfo.tunnelFd,
                 inet_ntoa(fdInfo.serverAddr.sin_addr), ntohs(fdInfo.serverAddr.sin_port));
    ...
    OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                 "🔍 [流量劫持检查] VPN启动时间: %{public}lld, 开始监控流量劫持情况", (long long)g_vpnStartTime);
    ...
    while (g_threadRunF) {
        ...
        if (g_packetsSent % 50 == 0) {
            ...
            OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", 
                         "🔍 [流量劫持检查] ========== 第%{public}d次检查 (VPN运行时间: %{public}lld秒) ==========",
        ...
        if (g_packetsSent > 10 && g_responsesReceived == 0) {
            ...
            if (g_lastResponseTime == 0 || (now - g_lastResponseTime) > 5) {
                ...
                OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "ZHOUB", "[VPN客户端] ⚠️ ⚠️ ⚠️ 警告：已发送 %{public}d 个数据包，但未收到任何响应！", g_packetsSent);
